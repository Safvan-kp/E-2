                    ...
                    if(item.StockId != l_ids[i].StockId && l_ids[i].StockId == p_ids[i].StockId )
                    {
                        ...
                        if(item.LocationId == l_ids[i].LocationId)
                        {
                            ...
                            if (item.ProductId == p_ids[i].ProductId)
                            {
                                ...
                                break;
                                //var Stocku = _e2DbContext.Stocks.Include(c => c.Product).Include(x => x.Location).FirstOrDefault(x => x.StockId == p_ids[i].StockId);
                                //var Stockd = _e2DbContext.Stocks.Include(c => c.Product).Include(x => x.Location).FirstOrDefault(x => x.StockId == item.StockId);
                                /*var Stockd = stock.Find(x => x.StockId == p_ids[i].StockId);
